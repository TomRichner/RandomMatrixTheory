...
% Parameters
n = 50;      % n
g = 2.5;              % gain (slightly chaotic when > 1)
...
amp = 0.5;
u(:,1:fix(nt/4)) = amp/2+amp.*repmat(randn(n,1), 1, fix(nt/4));
u(:,fix(nt/4)+(1:fix(nt/4))) = amp/2+amp.*repmat(randn(n,1), 1, fix(nt/4));
u(:,fix(nt/2)+(1:fix(nt/4))) = amp/2+amp.*repmat(randn(n,1), 1, fix(nt/4));
u(:,fix(0.75*nt)+(1:fix(nt/4))-1) = amp/2+amp.*repmat(randn(n,1), 1, fix(nt/4));
