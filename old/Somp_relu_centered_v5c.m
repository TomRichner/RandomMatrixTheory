...
plot_entropy_rate = true; % true: plot entropy production rate, false: plot full spectrum

% Parameters
n = 50;      % n
g = 2;              % gain (slightly chaotic when > 1)
...
% stim_change_interval = (tspan(2)-tspan(1))/5; % how often stimulus changes (in seconds)
stim_change_interval = 100; % how often stimulus changes (in seconds)
...
% stim_vectors(:,4) = stim_vectors(:,3);
% stim_vectors(:,5) = stim_vectors(:,3);

% stim_vectors2(:,1) = stim_vectors(:,2);
% stim_vectors2(:,2) = stim_vectors(:,3);
% stim_vectors2(:,3) = stim_vectors(:,3);
% stim_vectors2(:,4) = stim_vectors(:,1);
% stim_vectors2(:,5) = stim_vectors(:,1);
% stim_vectors2(:,6) = stim_vectors(:,1);

% stim_vectors = stim_vectors2;
...
if ~strcmpi(Lya_method, 'none')
    ...
    if strcmpi(Lya_method, 'qr')
        lya_dt = 50*dt;  % Longer interval for QR method
    else
...
if ~strcmpi(Lya_method, 'none')
    ...
    elseif strcmpi(Lya_method, 'qr')
        ...
        if plot_entropy_rate
            ...
            local_entropy_rate = sum(max(lya_results.local_LE_spectrum_t, 0), 2);
            mean_entropy_rate = sum(max(lya_results.LE_spectrum, 0));
            ...
            title(sprintf('Local Entropy Production Rate (Mean = %.4f)', mean_entropy_rate))
            legend('Local Entropy Rate', sprintf('Mean = %.3f', mean_entropy_rate), 'Location', 'best')
        else
