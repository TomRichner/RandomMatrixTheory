...
dt = 1/fs;
tspan = [-100, 400]*1;       % seconds
t = linspace(tspan(1), tspan(2), (tspan(2)-tspan(1))*fs+1)'; % time vector

nt = length(t);
m = 0.6; % 0.58 for a transient negative feedback
mu_E = 1;
mu_I = -1;
...
stim_vectors2(:,2) = stim_vectors(:,2);
stim_vectors2(:,3) = stim_vectors(:,3);
stim_vectors2(:,4) = stim_vectors(:,3);
stim_vectors2(:,5) = stim_vectors(:,1);
stim_vectors2(:,6) = stim_vectors(:,1);
% stim_vectors2(:,7) = stim_vectors(:,1);
...
if ~strcmpi(Lya_method, 'none')
    ...
    if strcmpi(Lya_method, 'qr')
        ...
        lya_dt = 10*dt;   % Standard interval for Benettin
    end
    lya_fs = 1/lya_dt;
...
if ~strcmpi(Lya_method, 'none')
    ...
    ylabel('Stimulus u(t)')
    ylim(1.2*ylim);
    % title('External Input Stimulus u(t)')
    ...
    if strcmpi(Lya_method, 'benettin')
        ...
        [bL,aL] = butter(5,0.04/(lya_fs/2),'low');
        local_lya_filt = filtfilt(bL,aL,lya_results.local_lya);
        % local_lya_filt = filter(bL,aL,lya_results.local_lya);
